...
function renderHeader() {
  ...
  return `<header class="header"><div><h1>Poker Nights Dashboard</h1><p>${getSeason().label}</p>${renderSeasonSwitcher()}</div><div class="badges"><span>${gamesToGo} nights to go</span><span>${nightsPlayed}/${getTotalTournamentNights()} nights played</span></div></header>`;
}
...
function renderBonuses() {
  const events = [];
  for (const game of getGames()) {
    if (game.handBonuses?.RF) events.push({game: game.id, night: game.dayId, player: game.handBonuses.RF, type: 'RF', points: getRfBonusPoints()});
    if (game.handBonuses?.SF) events.push({game: game.id, night: game.dayId, player: game.handBonuses.SF, type: 'SF', points: getSfBonusPoints()});
  }
...
function updateH2H() {
  ...
  if (!h2hA || !h2hB || !el) return;

  const a = h2hA.value;
  const b = h2hB.value;

  if (!a || !b) {
    el.innerHTML = '<p class="muted">Add at least two players for head to head.</p>';
    return;
  }
