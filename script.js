...
function getSeason() {
  ...
  if (!seasons[currentSeasonKey]) {
    currentSeasonKey = seasons.season2 ? "season2" : Object.keys(seasons)[0];
    if (currentSeasonKey) {
      localStorage.setItem("poker-season", currentSeasonKey);
    }
  }
...
function bonusFor(game, name) {
  ...
  if (game.handBonuses?.RF === name) pts += getRfBonusPoints();
  if (game.handBonuses?.SF === name) pts += getSfBonusPoints();
  return pts;
}
...
function h2h(playerA, playerB) {
  ...
  for (const game of games) {
    const a = game.pointsByPlayer[playerA];
    const b = game.pointsByPlayer[playerB];
...
function renderHeader() {
  ...
  const gamesToGo = Math.max(getTotalTournamentNights() - nightsPlayed, 0);
  ...
  return `
    <header class="header">
      ...
      <div class="badges">
        ...
        <span>${nightsPlayed}/${getTotalTournamentNights()} nights played</span>
      </div>
    </header>
  `;
}
...
function renderStandings() {
  ...
  if (!rows.length) {
    return card('Overall Standings', 'No players found for this season.', '<p class="muted">Check your data.js file.</p>');
  }
  ...
  return card(
    'Overall Standings',
    'Click-free static ranking',
    `
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>#</th><th>Player</th><th>Total</th><th>Gms</th><th>1st</th><th>2nd</th><th>3rd</th><th>Pod</th><th>PPG</th><th>RF</th><th>SF</th>
            </tr>
          </thead>
          <tbody>${body}</tbody>
        </table>
      ...
      <div class="mini">
        <span>Top performer: <b>${top.name}</b> (${top.totalPoints} pts)</span>
        <span>PPG: <b>${ppg.name}</b> (${ppg.avgPoints.toFixed(2)})</span>
...
function renderH2H() {
  ...
  return card(
    'Head to Head',
    'Games where both players finished on the podium.',
    `<div class="toolbar h2h"><select id="h2hA">${opts(a)}</select><b>VS</b><select id="h2hB">${opts(b)}</select></div><div id="h2hScore" class="score"></div>`
  );
}
...
function renderBonuses() {
  ...
  for (const game of games) {
    if (game.handBonuses?.RF) {
      events.push({ game: game.id, night: game.dayId, player: game.handBonuses.RF, type: 'RF', points: getRfBonusPoints() });
    }

    if (game.handBonuses?.SF) {
      events.push({ game: game.id, night: game.dayId, player: game.handBonuses.SF, type: 'SF', points: getSfBonusPoints() });
    }
  }

  const body = events.length
    ? `<div class="table-wrap small"><table><thead><tr><th>Game</th><th>Night</th><th>Player</th><th>Type</th><th>Pts</th></tr></thead><tbody>${events.map(e => `<tr><td>#${e.game}</td><td>${e.night}</td><td>${e.player}</td><td>${e.type}</td><td>${e.points}</td></tr>`).join('')}</tbody></table></div>`
    : '<p class="muted">No RF or SF recorded in game data yet.</p>';

  return card('RF / SF log', `Royal flush (+${getRfBonusPoints()}) and straight flush (+${getSfBonusPoints()}).`, body);
}
...
function updateH2H() {
  const aEl = document.getElementById('h2hA');
  const bEl = document.getElementById('h2hB');
  const el = document.getElementById('h2hScore');

  if (!aEl || !bEl || !el) return;
...
function bindEvents() {
  document.querySelectorAll(".season-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      currentSeasonKey = btn.dataset.season;
      localStorage.setItem("poker-season", currentSeasonKey);
      render();
    });
  });
  ...
  if (nightSelect) nightSelect.addEventListener('change', e => drawNight(e.target.value));
  if (placeSelect) placeSelect.addEventListener('change', e => drawPodium(e.target.value));
