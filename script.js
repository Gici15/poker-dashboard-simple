...
function compareRank(a, b) { return b.totalPoints - a.totalPoints || b.wins - a.wins || b.seconds - a.seconds || b.thirds - a.thirds || b.avgPoints - a.avgPoints || a.name.localeCompare(b.name); }
function maxNight() { return games.length ? Math.max(...games.map(g => g.dayId || 1)) : 1; }
...
function render() {
  ...
  drawCharts(); bindEvents(); updateH2H();
}
let charts = {};
function chartDefaults() {
  Chart.defaults.color = '#94a3b8'; Chart.defaults.borderColor = '#1f2937'; Chart.defaults.font.family = "system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
}
function makeChart(id, config) { if (charts[id]) charts[id].destroy(); charts[id] = new Chart(document.getElementById(id), config); }
function drawCharts() { chartDefaults(); drawCumulative(); drawNight(maxNight()); drawPodium(4); }
function drawCumulative() {
  const data = cumulativeRows();
  makeChart('cumulativeChart', {type:'line', data:{labels:data.map(r=>r.night), datasets:players.map((p,i)=>({label:p, data:data.map(r=>r[p]), borderColor:COLORS[i%COLORS.length], backgroundColor:COLORS[i%COLORS.length], tension:.35, pointRadius:0, borderWidth:2}))}, options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{position:'bottom'}, tooltip:{mode:'index', intersect:false}}, scales:{y:{beginAtZero:true, ticks:{precision:0}}, x:{grid:{display:false}}}}});
}
function nightRows(night) {
  return players.map(name => {
    let pts = 0, played = 0;
    games.filter(g => (g.dayId || 1) === night).forEach(g => { const v = g.pointsByPlayer[name]; if (typeof v === 'number') { pts += v + bonusFor(g, name); played++; }});
    return {name, pts, played};
  }).filter(r => r.played).sort((a,b) => a.pts - b.pts || a.name.localeCompare(b.name));
}
function drawNight(night) {
  const rows = nightRows(Number(night));
  makeChart('nightChart', {type:'bar', data:{labels:rows.map(r=>r.name), datasets:[{label:'Points', data:rows.map(r=>r.pts), backgroundColor:rows.map(r=>COLORS[players.indexOf(r.name)%COLORS.length]), borderRadius:10}]}, options:{indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{beginAtZero:true, ticks:{precision:0}}, y:{grid:{display:false}}}}});
}
function podiumRows(place) { return players.map(name => ({name, count: games.filter(g => g.pointsByPlayer[name] === Number(place)).length})).sort((a,b)=>a.count-b.count || a.name.localeCompare(b.name)); }
function drawPodium(place) {
  const rows = podiumRows(place);
  makeChart('podiumChart', {type:'bar', data:{labels:rows.map(r=>r.name), datasets:[{label:'Count', data:rows.map(r=>r.count), backgroundColor:rows.map(r=>COLORS[players.indexOf(r.name)%COLORS.length]), borderRadius:10}]}, options:{indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{x:{beginAtZero:true, ticks:{precision:0}}, y:{grid:{display:false}}}}});
}
function updateH2H() {
  ...
  if (a === b) { el.innerHTML = '<p class="muted">Select two different players.</p>'; return; }
  const r = h2h(a,b);
